...
```
┌────────────────────┐    ┌────────────────────┐    ┌────────────────────┐    ┌────────────────────┐
│ Sources            │    │ Refresh job        │    │ Artifact           │    │ Dashboard          │
│                    │    │                    │    │                    │    │                    │
│ GA4 Admin API      │ ─▶ │ refresh/main.py    │ ─▶ │ web/data.json      │ ─▶ │ index.html         │
│ GA4 Data API       │    │ per-tile fns,      │    │ 13 tile payloads   │    │ + Chart.js         │
│ BigQuery ISCHEMA   │    │ errors captured    │    │ + refresh meta     │    │ static, no build   │
└────────────────────┘    └─────────▲──────────┘    └────────────────────┘    └────────────────────┘
                                    │
                       GitHub Action — daily 06:00 UTC cron,
                       commits the data.json diff to the repo
```
